...
var RequestForms = React.createClass({
  ...
  handleSubmit: function(){
    if (this.state.terms && this.validationState() === 'success' && this.state.expiration !== 0) {
      var time = moment().add(this.state.expiration, 'd')._d;
      manifestApi.submitRequestForMedicalRecords(this.state.email, time, this.state.providerType, this.state.businessName, this.state.phoneNumber, this.state.urgent, this.state.providerFirstName, this.state.providerLastName, function(data){
        if (data) {
