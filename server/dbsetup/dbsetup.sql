...
CREATE TABLE IF NOT EXISTS `UNRdb`.`email` (
  `emailID` BIGINT(20) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '',
...
CREATE TABLE IF NOT EXISTS `UNRdb`.`Comment` (
  ...
  UNIQUE INDEX `ratingID_UNIQUE` (`shiftID` ASC)  COMMENT '',
  CONSTRAINT `shiftID`
    FOREIGN KEY ()
    REFERENCES `UNRdb`.`CompletedShift` ()
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
