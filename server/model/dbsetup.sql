...
CREATE TABLE IF NOT EXISTS `UNRdb`.`Facility` (
  ...
  `facilityEMR` VARCHAR(45) NOT NULL COMMENT '',
...
CREATE TABLE IF NOT EXISTS `UNRdb`.`Shift` (
  ...
  `shiftStatus` TINYINT(1) UNSIGNED NOT NULL DEFAULT 0 COMMENT '',
  ...
  `facilityPaid` TINYINT(2) UNSIGNED NOT NULL DEFAULT 0 COMMENT '',
  `userPaid` TINYINT(2) UNSIGNED NOT NULL DEFAULT 0 COMMENT '',
  ...
  PRIMARY KEY (`shiftID`, `shiftStatus`, `fk_Shift_facilityID`, `fk_Shift_specialtyID`)  COMMENT '',
...
CREATE TABLE IF NOT EXISTS `UNRdb`.`ShiftReviewOnUser` (
  `fk_ ShiftReviewOnUser_userID` INT UNSIGNED NOT NULL COMMENT '',
  `fk_ ShiftReviewOnUser_shiftID` BIGINT(20) UNSIGNED NOT NULL COMMENT '',
  `review` TINYINT(1) UNSIGNED NOT NULL COMMENT '',
  INDEX `fk_ ShiftReviewsOnUser_shiftID_idx` (`fk_ ShiftReviewOnUser_shiftID` ASC)  COMMENT '',
  INDEX `fk_ ShiftReviewsOnUser_userID_idx` (`fk_ ShiftReviewOnUser_userID` ASC)  COMMENT '',
  PRIMARY KEY (`fk_ ShiftReviewOnUser_userID`, `fk_ ShiftReviewOnUser_shiftID`)  COMMENT '',
  CONSTRAINT `fk_ ShiftReviewsOnUser_userID`
    FOREIGN KEY (`fk_ ShiftReviewOnUser_userID`)
    REFERENCES `UNRdb`.`User` (`userID`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `fk_ ShiftReviewsOnUser_shiftID`
    FOREIGN KEY (`fk_ ShiftReviewOnUser_shiftID`)
    REFERENCES `UNRdb`.`Shift` (`shiftID`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB;
