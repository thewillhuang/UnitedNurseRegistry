...
-- -----------------------------------------------------
-- Table `UNRdb`.`Referral`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `UNRdb`.`Referral` ;

CREATE TABLE IF NOT EXISTS `UNRdb`.`Referral` (
  `parent` INT UNSIGNED NOT NULL COMMENT '',
  `child` INT UNSIGNED NOT NULL COMMENT '',
  `child_facility` INT UNSIGNED NOT NULL COMMENT '',
  `path_length` TINYINT(2) UNSIGNED NOT NULL COMMENT '',
  `created` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '',
  INDEX `child_facility_idx` (`child_facility` ASC)  COMMENT '',
  INDEX `child_idx` (`child` ASC)  COMMENT '',
  PRIMARY KEY (`parent`, `child`, `child_facility`, `path_length`)  COMMENT '',
  INDEX `path_length` (`path_length` ASC)  COMMENT '',
  CONSTRAINT `parent`
    FOREIGN KEY (`parent`)
    REFERENCES `UNRdb`.`User` (`userID`)
    ON DELETE CASCADE
  ...
  CONSTRAINT `child`
    ...
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `child_facility`
    FOREIGN KEY (`child_facility`)
    REFERENCES `UNRdb`.`Facility` (`facilityID`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB;
