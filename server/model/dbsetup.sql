...
CREATE TABLE IF NOT EXISTS `UNRdb`.`Comment` (
  ...
  `fk_Comment_authorID` INT UNSIGNED NOT NULL COMMENT '',
  `comment` VARCHAR(140) NOT NULL COMMENT '',
  `posted` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '',
  `modified` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '',
  INDEX `fk_Comment_shiftID_idx` (`fk_Comment_shiftID` ASC)  COMMENT '',
  PRIMARY KEY (`commentID`, `fk_Comment_shiftID`, `fk_Comment_authorID`)  COMMENT '',
  INDEX `fk_Comment_userID_idx` (`fk_Comment_authorID` ASC)  COMMENT '',
  ...
  CONSTRAINT `fk_Comment_userID`
    FOREIGN KEY (`fk_Comment_authorID`)
    REFERENCES `UNRdb`.`User` (`userID`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB;
